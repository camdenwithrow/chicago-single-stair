...
const state = { candidates: null, neighborhoods: [], comparisons: [], metadata: null, map: null };
...
function initializeMap() {
  ...
  if (!window.SINGLE_STAIR_CONFIG?.protomapsUrl && !window.SINGLE_STAIR_CONFIG?.protomapsApiKey) {
    ...
    $("map-message").textContent = "Candidate data are shown without a basemap. Add a domain-restricted Protomaps API key or self-hosted tile URL in config.js.";
  }
}
...
function populateControls() {
  ...
  for (const id of ["scenario", "transit-distance", "zoning", "ownership", "vacant", "underbuilt", "high-need"]) $(id).addEventListener("change", applyFilters);
...
async function main() {
  try {
    [state.candidates, state.neighborhoods, state.comparisons, state.metadata] = await Promise.all([
      fetchGzipJson("data/candidates.geojson.gz"), fetch("data/neighborhoods.json").then((r) => r.json()),
